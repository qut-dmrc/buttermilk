{{
  config(
    materialized='table',
    description='Pipeline completeness issues for monitoring dashboard'
  )
}}

-- Monitor evaluation pipeline completeness for active experiments
-- Ensures FETCH -> JUDGE -> SYNTH -> SCORER -> DIFF flow completion
-- Returns rows when sessions have incomplete pipeline execution
...
SELECT 
  'Session Issue' as issue_type,
  judge_criteria,
  session_id as affected_item,
