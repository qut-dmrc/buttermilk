...
## LLMProcessor Architecture

**Critical Interface Mismatch**: LLMCore uses `process(record, *, processor_stage)` but pipelines use UnifiedProcessor with `process(context: ProcessingContext)`. LLMProcessor wraps LLMCore with the correct interface bridge.

Key implementation details:
- LLMProcessor inherits from UnifiedProcessor and implements `_process_record(context: ProcessingContext)`
- It internally creates LLMCore with the same configuration (model, prompt_template, temperature, max_tokens, output_col, template_vars)
- Supports `output_model` (Pydantic model path for structured output) and `fail_on_unfilled_parameters` flags
- LLMProcessor is exported from `buttermilk.processors.__init__.py` for pipeline use
- Do NOT use LLMCore directly in pipelines; always wrap it with LLMProcessor
