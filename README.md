...
**THE recommended way to initialize Buttermilk** is using `init_async()` in async contexts:
...
async def main():
    ...
    bm = await init_async(
        job="my_job",
        project="my_project",
        config_dir="conf",       # Path to your config directory
        config_name="config",    # Base config file name
        overrides=["llms=full"]  # Optional Hydra overrides
    )
    ...
    # Your code here
    logger.info("Buttermilk initialized!", session=bm.session_info.session_id)
...
**Why `init_async()` is THE way:**
- ✅ Single function call handles all bootstrap logic
- ✅ Returns BM instance with `bm.cfg` and `bm.logger` ready to use
- ✅ Proper async/await pattern for modern Python
- ✅ Session-scoped observability and resource management
- ❌ Don't use `init()` (sync wrapper, deprecated)
- ❌ Don't manually call `bootstrap_session_with_config_async()` (internal function)
