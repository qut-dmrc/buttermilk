...
PERMISSIONS=$(gcloud iam roles describe "$SOURCE_ROLE" --format="value(includedPermissions)" | tr ';' ',') gcloud iam roles update $ROLE --project=$GOOGLE_CLOUD_PROJECT --add-permissions=$PERMISSIONS
