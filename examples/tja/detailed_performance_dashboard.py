...
def init_bm():
    ...
    bm = init(job="detailed_performance_analysis", name="detailed_performance_dashboard", config_dir="../../buttermilk/conf")
    return bm
...
# Filter controls
selected_criteria = st.sidebar.multiselect("Select Criteria", available_criteria, default=available_criteria)

selected_models = st.sidebar.multiselect("Select Models", available_models, default=available_models)

selected_experiments = st.sidebar.multiselect("Select Experiments", available_experiments, default=available_experiments)

# Data quality filter
min_data_quality = st.sidebar.selectbox("Minimum Data Quality", ["ALL", "LIMITED", "SUFFICIENT"], index=1)
...
if not filtered_lift.empty:
    ...
    lift_display = filtered_lift[
        [
            ...
            "relative_accuracy_lift_pct",
            "lift_category",
            "effect_size",
            "synth_value_assessment",
        ]
    ].round(3)
    ...
    st.dataframe(lift_display.style.apply(highlight_lift, axis=1), use_container_width=True)
    ...
    if not matrix_filtered.empty:
        ...
        with col1:
            ...
            if selected_dimension == "Model_Agent":
                ...
                heatmap_data = matrix_filtered.pivot_table(index="dimension_1", columns="dimension_2", values="avg_accuracy", fill_value=0)
...
if not filtered_lift.empty and not filtered_individual.empty:
    ...
    with insights_col1:
        ...
        best_judge = (
            filtered_individual[filtered_individual["agent_role"] == "JUDGE"].loc[
                filtered_individual[filtered_individual["agent_role"] == "JUDGE"]["avg_accuracy"].idxmax()
            ]
            if not filtered_individual[filtered_individual["agent_role"] == "JUDGE"].empty
            else None
        )

        best_synth = (
            filtered_individual[filtered_individual["agent_role"] == "SYNTHESISER"].loc[
                filtered_individual[filtered_individual["agent_role"] == "SYNTHESISER"]["avg_accuracy"].idxmax()
            ]
            if not filtered_individual[filtered_individual["agent_role"] == "SYNTHESISER"].empty
            else None
        )
