...
def run_flow(
    *,
    ...
    run_cfg,
    step_outputs: str | pd.DataFrame | None = None,
...
def exec_local(
    *,
    ...
    try:
        ...
        inputs = pd.Series(
            inputs[columns.keys()].to_dict(orient="records"),
            index=inputs.index,
            name="inputs",
        )
        ...
        relevant_batch_cols = [
            ...
            if c in results.columns and c not in batch_id.keys()
        ]
        batch_columns = pd.json_normalize(itertools.repeat(batch_id, results.shape[0]))
        batch_columns = pd.concat(
            [results[relevant_batch_cols].reset_index(drop=True), batch_columns],
            axis="columns",
        )
    ...
    try:
        for step_name, step_cfg in cfg.experiments.items():
            ...
            try:
                ...
                logger.info(
                    dict(
                        message=f"Completed step: {step_name}, processed {df.shape[0]} results in {format_timespan(t1 - t0)}. Saved to {uri}",
                        results=uri,
                    ),
                )
...
def run(*, flow_name, flow_cfg, flow_obj, run_cfg, run_names: dict = {}):
    ...
    df.loc[:, "run_id"] = bm.run_info.run_id
