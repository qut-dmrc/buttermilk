...
class TestAgentWithTools(Agent):
    ...
    async def _process(self, *, message: AgentInput, **kwargs: Any) -> AgentOutput:
        ...
        if operation == "analyze":
            ...
            result = {"query": query, "results": [f"result_{i}" for i in range(min(limit, 5))], "total": min(limit, 5)}
        else:
    ...
    async def analyze_data(self, data: str, format: str = "json") -> dict[str, Any]:
        ...
        return {"analysis": f"Analyzed {len(data)} characters", "format": format, "insights": ["insight1", "insight2"]}

    @tool(name="search_documents")
    def search(self, query: str, limit: int = 10) -> dict[str, Any]:
        """Search for documents."""
        return {"query": query, "results": [f"result_{i}" for i in range(min(limit, 5))], "total": min(limit, 5)}
...
class TestLLMAgentWithTools(LLMAgent):
    ...
    def classify_text(self, text: str) -> dict[str, Any]:
        ...
        return {"text": text, "category": "general", "confidence": 0.95}
...
class TestToolDefinitionIntegration:
    ...
    def test_agent_tool_extraction(self):
        ...
        agent = TestAgentWithTools(agent_name="test_agent", model_name="test", role="analyzer")
    ...
    def test_llm_agent_tool_extraction(self):
        ...
        agent = TestLLMAgentWithTools(agent_name="llm_agent", model_name="test-model", role="assistant", parameters={"model": "test-model"})
    ...
    async def test_structured_host_with_agent_tools(self):
        ...
        # Create test agents
        agent1 = TestAgentWithTools(agent_name="analyzer", model_name="test", role="ANALYZER")

        agent2 = TestLLMAgentWithTools(agent_name="assistant", model_name="test-model", role="ASSISTANT", parameters={"model": "test-model"})

        # Create host
        host = StructuredLLMHostAgent(agent_name="host", model_name="test-model", role="HOST", parameters={"model": "test-model"})

        # Mock initialization
        host._participants = {"ANALYZER": agent1, "ASSISTANT": agent2}
    ...
    async def test_backward_compatibility(self):
        ...
        class LegacyAgent(Agent):
            ...
            async def _process(self, *, message: AgentInput, **kwargs: Any) -> AgentOutput:
                return AgentOutput(agent_id=self.agent_id, outputs={"message": "Legacy response"})

        agent = LegacyAgent(agent_name="legacy", model_name="test", role="LEGACY")
    ...
    async def test_mixed_sync_async_tools(self):
        ...
        agent = TestAgentWithTools(agent_name="test", model_name="test", role="test")
