...
def slack_app():
    ...
    app.client.conversations_replies = AsyncMock(return_value={
        "messages": [
...
async def test_slack_ui_agent_initialization(slack_ui_agent):
    ...
    with patch("buttermilk.agents.ui.slackthreadchat.register_chat_thread_handler") as mock_register:
        await slack_ui_agent.initialize(input_callback=input_callback)

    mock_register.assert_called_once_with(slack_ui_agent.context.thread_ts, slack_ui_agent)
...
async def test_slack_ui_agent_send_to_thread(slack_ui_agent):
    ...
    with patch("buttermilk.agents.ui.slackthreadchat.post_message_with_retry", new_callable=AsyncMock) as mock_post:
...
async def test_slack_ui_agent_receive_output_agent_output(slack_ui_agent):
    ...
    with patch("buttermilk.agents.ui.slackthreadchat.format_slack_message") as mock_format:
...
async def test_process_method(slack_ui_agent):
    ...
    with patch.object(slack_ui_agent, "_request_user_input", new_callable=AsyncMock) as mock_request:
