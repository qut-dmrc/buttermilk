...
class TestStructuredDebate:
    ...
    async def test_structured_debate_flow_phases(self):
        ...
        selector._get_host_suggestion.side_effect = [
            StepRequest(role="JUDGE", prompt=f"Provide initial assessment of the content", description=f"Phase 1: {phases[0]}"),
            StepRequest(role="HOST", prompt=f"Analyze judge responses to identify consensus and disagreements", description=f"Phase 2: {phases[1]}"),
            StepRequest(role="HOST", prompt=f"Create plan to resolve disagreements", description=f"Phase 3: {phases[2]}"),
            StepRequest(role="JUDGE", prompt=f"Focus on resolving specific disagreement 1", description=f"Phase 4: {phases[3]} - Point 1"),
            # Add a user interrupt here
            StepRequest(role="JUDGE", prompt=f"Focus on resolving specific disagreement 2", description=f"Phase 4: {phases[3]} - Point 2"),
            StepRequest(role="SYNTHESISER", prompt=f"Synthesize results into comprehensive answer", description=f"Phase 5: {phases[4]}"),
            StepRequest(role="JUDGE", prompt=f"Evaluate the synthesized answer", description=f"Phase 6: {phases[5]}"),
            StepRequest(role="HOST", prompt=f"Present final answer to manager", description=f"Phase 7: {phases[6]}"),
        ...
        steps_executed_before_interrupt = sum(1 for i, call in enumerate(selector._execute_step.call_args_list) 
                                             if i < interrupted_step_index)
