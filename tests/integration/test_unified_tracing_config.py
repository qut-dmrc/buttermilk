...
class TestUnifiedTracingConfig:
    ...
    def weave_config(self) -> Dict[str, Any]:
        ...
        return {"enabled": True, "project_id": "test-dmrc", "api_key": "fake-wandb-api-key-for-testing"}

    @pytest.fixture
    def traceloop_config(self) -> Dict[str, Any]:
        """Valid traceloop tracing configuration."""
        return {"enabled": True, "api_key": "fake-traceloop-api-key-for-testing", "endpoint": "https://api.traceloop.com"}

    @pytest.fixture
    def otel_config(self) -> Dict[str, Any]:
        """Valid OTEL tracing configuration."""
        return {"enabled": True, "endpoint": "https://telemetry.googleapis.com", "otlp_headers": {}}
    ...
    def unified_tracing_config(self, weave_config, traceloop_config, otel_config) -> Dict[str, Dict[str, Any]]:
        ...
        return {"weave": weave_config, "traceloop": traceloop_config, "otel": otel_config}
    ...
    async def test_weave_config_based_initialization(self, weave_config):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            with patch("weave.init") as mock_weave_init, patch("weave.get_client") as mock_get_client:
                ...
                ctx = ExecutionContext(clouds=[], tracing=tracing_config)
    ...
    async def test_multiple_providers_configuration(self, unified_tracing_config):
        ...
        tracing_config = {provider: Tracing(**config) for provider, config in unified_tracing_config.items()}
...
class TestTracingConfigExamples:
    ...
    async def test_minimal_weave_setup_example(self):
        ...
        with patch("weave.init") as mock_init, patch("weave.get_client") as mock_get_client:
    ...
    async def test_production_multi_provider_example(self):
        ...
        production_config = {
            "weave": {"enabled": True, "project_id": "research-production", "api_key": "prod-wandb-key"},
            "traceloop": {"enabled": True, "api_key": "prod-traceloop-key", "endpoint": "https://api.traceloop.com"},
            "otel": {"enabled": True, "endpoint": "https://telemetry.googleapis.com", "otlp_headers": {"custom-header": "production-value"}},
        }
