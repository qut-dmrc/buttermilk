...
class TestBMSessionLoggingProtection:
    """Test logging protection during BM session creation."""
    ...
    def test_multiple_bm_sessions_logging_protection(self, mock_setup_file, mock_setup_console, tmp_path):
        ...
        # Attempt to create second ExecutionContext should fail fast
        with pytest.raises(RuntimeError) as exc_info:
            create_execution_context()
        
        assert "ExecutionContext has already been initialized" in str(exc_info.value)
        
        # But we can create another BM session safely using existing context
        bm2 = create_session_bm(
            name="test_session_2",
            job="test_job_2",
            platform="test",
            save_dir_base=str(tmp_path),
    ...
    def test_safe_execution_context_reuse_pattern(self, mock_setup_file, mock_setup_console, tmp_path):
        ...
        # First session using safe pattern
        ...
        # Second session using safe pattern
...
class TestFailFastIntegrationExamples:
    ...
    def test_recommended_application_startup_pattern(self, mock_setup_file, mock_setup_console, tmp_path):
        ...
        # Step 1: Initialize ExecutionContext once at application startup
        execution_context = get_or_create_execution_context()
        
        # Step 2: Create multiple BM sessions safely
        bm_sessions = []
        for i in range(3):
            session = create_session_bm(
                name=f"session_{i}",
                job=f"job_{i}",
                platform="test",
                save_dir_base=str(tmp_path),
        ...
        # Step 5: All subsequent ExecutionContext access is safe
        for _ in range(5):
            same_context = get_or_create_execution_context()
            assert same_context is execution_context
