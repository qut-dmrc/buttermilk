...
@pytest.fixture
def real_scorer_config():
    """Get the SCORER agent configuration.

    This returns the expected configuration for the scorer agent,
    matching the structure the tests expect to verify robust extraction logic.
    """
    return {
        "inputs": {
...
class TestAgentVariableExtraction:
    ...
    def test_extract_record_from_real_fetch_output(self, real_scorer_config, sample_record_with_ground_truth: Record):
        ...
        assert "ground_truth" in source_dict, f"ground_truth not found in extracted source. Keys: {source_dict.keys()}"
...
class TestScorerConfigWithRealTraceData:
    ...
    def test_scorer_source_extraction_from_fetch(self, real_scorer_config, scorer_input_trace):
        ...
        assert result is not None, f"Source extraction failed!\nExpression: {source_expr}\nAvailable keys: {list(scorer_input_trace.keys())}"
    ...
    def test_scorer_expected_extraction_from_fetch(self, real_scorer_config, scorer_input_trace):
        ...
        assert result is not None, (
            f"Expected extraction failed!\nExpression: {expected_expr}\nFETCH.outputs keys: {list(scorer_input_trace['FETCH']['outputs'].keys())}"
        )
    ...
    def test_scorer_answers_extraction_from_judge(self, real_scorer_config, scorer_input_trace):
        ...
        assert result is not None, f"Answers extraction failed!\nExpression: {answers_expr}"
    ...
    def test_scorer_instructions_extraction_from_judge(self, real_scorer_config, scorer_input_trace):
        ...
        assert result is not None, (
            f"Instructions extraction failed!\nExpression: {instructions_expr}\nJUDGE.messages: {scorer_input_trace['JUDGE'].get('messages', [])}"
        )
    ...
    def test_all_scorer_inputs_extract_successfully(self, real_scorer_config, scorer_input_trace):
        ...
        if failures:
            pytest.fail(
                "Some scorer inputs failed to extract:\n"
    ...
    def test_fallback_patterns_for_source(self, real_scorer_config, scorer_input_trace):
