...
async def test_iterative_rag_agent_can_call_tool_multiple_times(mocker):
    """Test that the RagAgent can call tools multiple times in one session."""
    ...
    mock_llm_client.call_chat = AsyncMock(
        side_effect=[
            # First call: LLM decides to use the search tool
            CreateResult(
                content="",
                thought="Thinking about initial search",
                finish_reason="function_calls",
                usage=RequestUsage(prompt_tokens=10, completion_tokens=10),
                cached=False,
                tool_calls=[MockToolCall(id="call_1", function={"name": "search_tool", "arguments": '{"query": "initial query"}'})],
            ),
            # Second call: LLM decides to use the search tool again
            CreateResult(
                content="",
                thought="Refining search",
                finish_reason="function_calls",
                usage=RequestUsage(prompt_tokens=10, completion_tokens=10),
                cached=False,
                tool_calls=[MockToolCall(id="call_2", function={"name": "search_tool", "arguments": '{"query": "follow-up query"}'})],
            ),
            # Final call: LLM synthesizes the answer
            CreateResult(
                content="Final answer based on two searches",
