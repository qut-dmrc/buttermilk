...
import buttermilk.utils.cli as cli_module
...
class TestCliInit:
    ...
    def test_cli_init_basic_functionality(self, mock_asyncio_run, mock_set_bm, mock_bootstrapper_class):
        ...
        # Test basic call
        result = cli.init(job="test_job", name="test_session")
        ...
        with patch('buttermilk.utils.cli.asyncio.run') as mock_asyncio_run:
            ...
            with patch('buttermilk.utils.cli.set_bm'):
                ...
                with patch('buttermilk.utils.cli.Path.cwd') as mock_cwd:
                    ...
                    cli.init(job="test_job", path=custom_path)
    ...
    def test_cli_init_overrides_handling(self, mock_bootstrapper_class):
        ...
        with patch('buttermilk.utils.cli.asyncio.run') as mock_asyncio_run:
            ...
            with patch('buttermilk.utils.cli.set_bm'):
                ...
                cli.init(job="test_job", overrides=custom_overrides)
...
class TestCliInitWithConfig:
    """Test the CLI init_with_config() function."""
    
    @patch('buttermilk.utils.cli.ConfigurationBootstrapper')
    @patch('buttermilk.utils.cli.set_bm')
    @patch('buttermilk.utils.cli.asyncio.run')
    def test_init_with_config_basic_functionality(self, mock_asyncio_run, mock_set_bm, mock_bootstrapper_class):
        """Test basic init_with_config functionality."""
        # Setup mocks
        mock_bootstrapper = MagicMock()
        mock_bootstrapper_class.return_value = mock_bootstrapper
        ...
        # Test basic call
        bm_result, config_result = cli.init_with_config(job="test_job", name="test_session")
    ...
    @patch('buttermilk.utils.cli.asyncio.run')
    @patch('buttermilk.utils.cli.sys.modules')
    def test_init_with_config_name_from_script(self, mock_sys_modules, mock_asyncio_run, mock_set_bm, mock_bootstrapper_class):
        """Test init_with_config with automatic name extraction."""
        # Setup mocks
        mock_bootstrapper = MagicMock()
        mock_bootstrapper_class.return_value = mock_bootstrapper
        mock_bm = MagicMock(spec=BM)
        ...
        # Test without explicit name
        bm_result, config_result = cli.init_with_config(job="test_job")
    ...
    @patch('buttermilk.utils.cli.ConfigurationBootstrapper')
    def test_init_with_config_error_handling(self, mock_bootstrapper_class):
        """Test error handling in init_with_config."""
        # Setup bootstrapper to raise exception
        mock_bootstrapper = MagicMock()
        mock_bootstrapper_class.return_value = mock_bootstrapper
        ...
        with patch('buttermilk.utils.cli.asyncio.run') as mock_asyncio_run:
            ...
            with patch('buttermilk.utils.cli.logger') as mock_logger:
                with pytest.raises(Exception, match="Configuration error"):
                    cli.init_with_config(job="test_job")
...
"""
        ...
        with patch('buttermilk.utils.cli.logger'):
            bm_instance = cli.init(
                job="cli_integration_test",
                name="cli_test_session",
                path=str(config_dir)
            )
    ...
    def test_init_with_config_integration_with_real_config(self, real_conf, tmp_path):
        """Test init_with_config with real configuration."""
...
"""
        ...
        with patch('buttermilk.utils.cli.logger'):
            bm_instance, config = cli.init_with_config(
                job="cli_config_test",
                name="cli_config_session",
                path=str(config_dir)
            )
            ...
            with patch('buttermilk.utils.nb.ConfigurationBootstrapper') as mock_nb_bootstrap:
                with patch('buttermilk.utils.cli.asyncio.run'), patch('buttermilk.utils.nb.asyncio.run'):
                    with patch('buttermilk.utils.cli.set_bm'), patch('buttermilk.utils.nb.set_bm'):
                        ...
                        # Test CLI init
                        cli.init(job="test_job", path="/test/path")
