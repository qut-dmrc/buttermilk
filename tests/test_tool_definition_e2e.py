...
class ResearchAgent(LLMAgent):
    ...
    async def search_documents(self, query: str, max_results: int = 10) -> dict[str, Any]:
        ...
        return {
            "query": query,
            "results": [{"id": f"doc_{i}", "title": f"Document {i}", "relevance": 0.9 - i * 0.1} for i in range(min(max_results, 3))],
    ...
    async def analyze_document(self, doc_id: str) -> dict[str, Any]:
        ...
        return {"doc_id": doc_id, "summary": f"Summary of {doc_id}", "key_points": ["Point 1", "Point 2", "Point 3"], "sentiment": "neutral"}
...
class WriterAgent(Agent):
    ...
    async def _process(self, *, message: AgentInput, **kwargs: Any) -> AgentOutput:
        ...
        return AgentOutput(
            agent_id=self.agent_id, outputs={"written_content": f"[{style.upper()}] {content}", "word_count": len(content.split()), "style": style}
        )
    ...
    def format_content(self, text: str, format_type: str = "markdown") -> dict[str, str]:
        ...
        formatted = {"markdown": f"# {text}\n\nFormatted as markdown.", "html": f"<h1>{text}</h1><p>Formatted as HTML.</p>", "plain": text}
        return {"original": text, "formatted": formatted.get(format_type, text), "format": format_type}
...
class TestEndToEndFlows:
    ...
    def mock_agents(self):
        ...
        writer = WriterAgent(agent_name="writer", model_name="test-model", role="WRITER")

        return {"RESEARCHER": research, "WRITER": writer}
    ...
    async def test_complete_research_flow(self, mock_agents):
        ...
        host = StructuredLLMHostAgent(
            agent_name="host", model_name="test-model", role="HOST", parameters={"model": "test-model", "template": "host_structured_tools"}
        )
