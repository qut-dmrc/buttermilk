...
class TestBootstrapQueryRunnerInjection:
    ...
    async def test_bootstrap_creates_bm_with_query_runner(self, real_conf):
        ...
        execution_context, infrastructure = await bootstrapper.bootstrap_full_context()
        ...
        session_bm = await bootstrapper.bootstrap_session_context(
            ...
            infrastructure=infrastructure  # Use existing infrastructure like dashboard does
        )
    ...
    async def test_bootstrap_session_bm_has_run_query_convenience_method(self, real_conf):
        ...
        execution_context, infrastructure = await bootstrapper.bootstrap_full_context()
        session_bm = await bootstrapper.bootstrap_session_context(
            name="streamlit_dashboard_pattern",
            job="tja_template_analysis_pattern",
            infrastructure=infrastructure
        )
    ...
    async def test_bootstrap_infrastructure_sharing_preserves_query_runner(self, real_conf):
        ...
        session_bm_1 = await bootstrapper.bootstrap_session_context(
            name="shared_infra_test_1",
            job="session_1",
            infrastructure=infrastructure
        )
        
        session_bm_2 = await bootstrapper.bootstrap_session_context(
            name="shared_infra_test_2", 
            job="session_2",
            infrastructure=infrastructure
        )
