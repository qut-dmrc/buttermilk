...
def test_client_type_enum():
    ...
    expected_types = ["openai", "azure", "anthropic", "anthropic_vertex", "gemini", "gemini_vertex", "vertex_openai"]
...
def test_llm_config_validates_client_type():
    ...
    model_info = ModelInfo(family="test-family", function_calling=True, json_output=True, vision=False, structured_output=False)

    # Valid client type as string
    config = LLMConfig(client_type="openai", api_key="test-key", model_info=model_info, configs={"model": "gpt-4"})
    assert config.client_type == ClientType.OPENAI

    # Valid client type as enum
    config = LLMConfig(
        client_type=ClientType.AZURE, api_key="test-key", base_url="https://example.azure.com", model_info=model_info, configs={"model": "gpt-4"}
    )
    ...
    with pytest.raises(ValueError, match="Unsupported client_type"):
        LLMConfig(client_type="invalid_type", api_key="test-key", model_info=model_info, configs={})
