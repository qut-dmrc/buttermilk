...
export async function refetchRecords() {
  ...
  if (currentFlow && currentDataset && currentDataset.trim() !== "") {
    ...
    const endpoint = `/api/flows/${encodeURIComponent(currentFlow)}/datasets/${
      encodeURIComponent(currentDataset)
    }/records`
...
export async function loadParametersFromSessions() {
  try {
    ...
    console.log("Demo mode: Loaded parameters from sessions", {
      flows: flows.size,
      datasets: datasets.size,
      records: records.size,
      criteria: criteriaSet.size,
    })
  } catch (error) {
