<script lang="ts">
	...
	function getPanelState(
		...
		panel: 'details' | 'reasons' | 'assessments'
	): boolean {
	...
	$: summaryMessages = messages.filter(
		(m) => m.type === 'summary_result' || (m.outputs && isSummaryResult(m.outputs))
	);

	// Get only Judge messages for the sidebar when no summary is available
	$: judgeMessages = messages.filter(
		(m) => m.outputs && m.outputs.prediction !== undefined && m.outputs.conclusion !== undefined
	);
		...
		<div class="sidebar-content">
			...
			{#if summaryMessages.length > 0}
				{#each summaryMessages as message, i (message.message_id ? `${message.message_id}_${i}` : `summary_${i}_${Date.now()}_${Math.random().toString(36).substring(2, 10)}`)}
					...
					<div class="summary-entry" in:fly={{ y: 20, duration: 300, delay: i * 50 }}>
						<!-- Summary Title -->
						...
						<div class="summary-stats">
										...
										: summaryData.predictions && summaryData.predictions.length > 1
											? `${Math.round(
													(Math.max(
														...
														summaryData.predictions.length) *
														100
												)}%`
											: 'N/A'}
								</span>
							...
							<div class="stat-row">
								...
								<span class="stat-value" style="color: {getScoreColor(summaryData.avg_score)}">
									{summaryData.avg_score ? formatPercent(summaryData.avg_score) : 'N/A'}
								...
								{#each summaryData.predictions as prediction}
									...
									<div class="prediction-item">
										...
										<span class="pred-agent">
											{prediction.agent_info?.name ||
												safeSubstring(prediction.agent_info?.agent_id, 0, 8)}
										</span>
			...
			{:else if judgeMessages.length > 0}
				{#each judgeMessages as message, i (message.message_id ? `${message.message_id}_${i}` : `judge_${i}_${Date.now()}_${Math.random().toString(36).substring(2, 10)}`)}
					...
					<div class="judge-entry" in:fly={{ y: 20, duration: 300, delay: i * 50 }}>
									...
									{#if getPanelState(message.message_id, 'assessments')}
										<div class="score-chips" transition:slide={{ duration: 150 }}>
											{#each scores as score}
												{@const scoreValue =
													score.outputs?.correctness !== undefined
														? score.outputs.correctness
														: null}
												{@const scoreColor = getScoreColor(scoreValue)}
												{@const hasReasons =
													score.outputs?.assessments && score.outputs.assessments.length > 0}
												...
												{#each [score] as scoreItem}
													...
													<div
														...
														on:click={() => {
															...
															expandedPanels[detailKey] = expandedPanels[detailKey]
																? {
																		...expandedPanels[detailKey],
																		details: !expandedPanels[detailKey].details
																	}
													...
													>
														<span class="terminal-score-chip">
															<span class="agent-tag" style="color: {scoreColor};"
																>{identifier}</span
															>
															...
															{#if hasReasons}
																<i
																	class="bi bi-chevron-{expandedPanels[
																		`${message.message_id}_${score.message_id}`
																	]?.details
																		? 'up'
																		: 'down'} expand-indicator"
																></i>
															{/if}
														</span>
													</div>
