<script lang="ts">
	...
	$: agentEmoji = getAgentEmoji(
		message.agent_info?.agent_name || 'System',
		...
		<svelte:fragment slot="messagePrefix">
			...
			{#if message.outputs?.metadata}
				<span class="metadata-inline">
					{#if message.outputs.metadata.date}| date: {formatDate(
							message.outputs.metadata.date
						)}{/if}
				...
				<span class="record-inline">
					...
					{#if message.outputs?.metadata?.url}
						<div class="url-line">
							...
							<a
								href={message.outputs.metadata.url}
								target="_blank"
								rel="noopener noreferrer"
								class="url-link">{message.outputs.metadata.url}</a
							>
						</div>
					{/if}
				</span>
				<div
					class="{isContentExpanded ? 'expanded-content' : 'truncated-content'} markdown-content"
				>
			...
			<div class="card-body p-2">
				<div class="tiny-text">
					...
					{#if message.type === 'judge_reasons' && message.outputs?.conclusion}
						<div>
							Conclusion: <span class="text-muted text-truncate">{message.outputs.conclusion}</span>
						</div>
					{/if}
				</div>
			</div>
