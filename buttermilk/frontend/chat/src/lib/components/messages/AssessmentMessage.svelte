	...
	<div class="sidebar-card mt-2 mb-3">
		<div class="card">
			...
			>
				...
				>
					<i class="bi bi-star-fill" style="color: {getScoreColor(message.outputs.correctness)};"
					></i>
				</span>
			</div>
			...
			<div class="card-body p-2">
				<div class="tiny-text">
					<div>
						Assessed: <span class="text-muted"
							>{message.outputs.assessed_agent_id.split('-')[0]}</span
						>
					</div>
					<div>
						Score: <span style="color: {getScoreColor(message.outputs.correctness)};"
							>{message.outputs.score_text}</span
						>
					</div>
