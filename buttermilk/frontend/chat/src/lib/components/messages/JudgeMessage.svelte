<script lang="ts">
	...
	$: {
		...
		assessments = $messageStore.filter(
			(m) => m.type === 'assessments' && m.outputs?.assessed_call_id === message.message_id
		);

		// Log the result of the filtering
		console.debug(
			`[JudgeMessage ${message.message_id}] Filtered assessments (${assessments.length} found):`,
			assessments
		);
	}
	...
	onMount(() => {
		if (browser && tooltipElement) {
			// Check if in browser and element is bound
	...
	$: uncertainty = message.outputs?.uncertainty
		? formatUncertainty(message.outputs.uncertainty)
		: undefined;
			...
			<div class="card-body p-2">
				<div class="tiny-text">
					{#if message.agent_info?.parameters?.model}
						<div>
							Model: <span style="color: {modelBasedColor};"
								>{message.agent_info?.parameters?.model}</span
							>
						</div>
					{:else}
						<div>
							Agent ID: <span class="text-muted">{message.agent_info?.agent_id || 'Unknown'}</span>
						</div>
					{/if}

					<div>
						Uncertainty: <span style="color: {uncertainty?.color};"
							>{uncertainty?.text || 'N/A'}</span
						>
					</div>
					...
					{#if message.agent_info?.parameters?.criteria}
						<div>
							Criteria: <span class="text-muted">{message.agent_info.parameters.criteria}</span>
						</div>
					{/if}
				</div>
			</div>
