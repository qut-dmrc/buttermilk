...
export const AGENT_STYLES = {
	...
	diff: { color: '#FF6347', background: '#ffe6e1', border: '#ffb3a6' } // Tomato for DIFF agents
};
...
export function getModelColor(modelName: string | undefined): string {
	...
	if (
		...
		return '#ff6b35'; // Anthropic orange
...
export function formatUncertainty(uncertainty: string) {
	...
	if (level === 1)
		color = '#28a745'; // Green for low uncertainty
...
export function isSystemUpdate(data: any): data is SystemUpdate {
	return typeof data === 'object' && data !== null && data.type === 'system_update';
}
...
export function isRecord(data: any): data is RecordData {
	return typeof data === 'object' && data !== null && data.type === 'record';
}

// Type guard for Researcher data
export function isResearcher(data: any): data is ResearcherData {
	return typeof data === 'object' && data !== null && data.type === 'researcher';
}
	...
	return (
		...
		data.divergences.every(
			(div: any) =>
				...
				div.positions.every(
					(pos: any) => typeof pos.position === 'string' && Array.isArray(pos.experts)
				)
		)
	);
}
