...
export function clearAllMessageHistory(): void {
	...
	try {
		...
		const messageKeys = keys.filter(
			(key) => key.startsWith('messageHistory_') || key === 'messageHistory'
		);
		...
		const buttermilkKeys = keys.filter(
			(key) => key.includes('buttermilk') || key.includes('session') || key.includes('flow')
		);
