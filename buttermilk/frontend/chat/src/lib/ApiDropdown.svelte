<script lang="ts">
	...
	onMount(() => {
		...
		if (isApiStore && hasFetchWithCache) {
			...
			console.log(
				`  Skipping initial fetchWithCache for "${label}" (not an API store or method missing)`
			);
		}
	...
	function getOptionValue(item: any): string {
		...
		if (isPlainArray) {
			...
			if (label === 'Select Record') {
				console.log(
					`Record option processing: item=${JSON.stringify(item)}, valueProperty=${valueProperty}, propValue=${propValue}`
				);
			}
