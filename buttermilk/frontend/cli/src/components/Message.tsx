...
import { AgentOutput, FlowProgressUpdate, Message } from "../types.js"
        ...
        {message.type === "ui_message" && (
          <Box flexDirection="column" paddingTop={0}>
            <Text color={retroIRCTheme.colors.textDim} dimColor>
              [DEBUG] UIMessage - hasOptions: {!!(message as any).outputs?.options ? "YES" : "NO"}, optionsType:{" "}
              {typeof (message as any).outputs?.options}, isArray:{" "}
              {Array.isArray((message as any).outputs?.options) ? "YES" : "NO"}
            </Text>
          </Box>
        )}
