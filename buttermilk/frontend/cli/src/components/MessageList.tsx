...
const MessageList = ({ messages }: Props) => {
    ...
    return (
      <Box flexDirection="column" paddingTop={1}>
        <Text color={retroIRCTheme.colors.textDim}>
          {retroIRCTheme.format.timestamp(new Date())} {retroIRCTheme.format.nickname("System", 15)}
          {retroIRCTheme.layout.nickSeparator} Welcome to Buttermilk Terminal
        </Text>
        <Text color={retroIRCTheme.colors.textDim}>
          {retroIRCTheme.format.timestamp(new Date())} {retroIRCTheme.format.nickname("System", 15)}
          {retroIRCTheme.layout.nickSeparator} Type /help for available commands
        </Text>
      </Box>
    )
  }
  ...
  return (
    <Box flexDirection="column">
      {filteredMessages.map((msg, i) => <MessageComponent key={i} message={msg} />)}
    </Box>
  )
}
