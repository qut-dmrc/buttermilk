...
class FlowClient {
  ...
  private handleMessage(msg: any): void {
    ...
    switch (msg.type) {
      ...
      case "agent_output":
        console.log(
          `🤖 ${msg.payload?.agent_id || "Agent"}: ${
            msg.payload?.content || JSON.stringify(msg.payload).substring(0, 200)
          }`,
        )
        break
      ...
      case "osb_partial":
        console.log(
          `🔬 OSB ${msg.payload?.agent}: ${msg.payload?.content || JSON.stringify(msg.payload).substring(0, 200)}`,
        )
        break
