                ...
                if message.inputs and message.inputs.records:
                    ...
                    for record in message.inputs.records:
                        if hasattr(record, "text") and hasattr(record, "content"):
                            logger.warning(
                                f"SpyAgent '{self.name}': Record '{record.record_id}' has both 'text' and 'content' fields. "
                                "Excluding 'text' field before saving."
                            )
                            # Create a new dict from model_dump excluding 'text'
                            modified_records.append(record.model_dump(exclude={"text"}))
                            needs_modification = True
                        else:
                            modified_records.append(record) # Or record.model_dump() if consistency needed
                    
                    if needs_modification:
                        # Re-assign records in a way that Pydantic recognizes the change if necessary.
                        # This might involve creating a new AgentInput if it's immutable,
                        # or directly setting if mutable and Pydantic handles it.
                        # For simplicity, assuming direct modification or that message object is mutable here.
                        # However, Pydantic models are generally immutable unless configured otherwise.
                        # A safer approach if AgentInput/Record are immutable and validated:
                        # new_inputs = message.inputs.model_copy(update={"records": modified_records})
                        # message = message.model_copy(update={"inputs": new_inputs})
                        # For now, direct modification as in original:
                        message.inputs.records = modified_records # type: ignore


                await self.manager.add(message) # Add message to the uploader's buffer
            else:
