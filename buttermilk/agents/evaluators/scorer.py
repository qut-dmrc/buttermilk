...
from typing import Any, Type # For type hinting and class types
...
class LLMScorer(LLMAgent):
    ...
    async def _listen(
        ...
        **kwargs: Any,
        ...
        if "records" not in extracted_data or "answers" not in extracted_data:
            logger.warning(
                f"Scorer '{self.agent_name}': Could not extract 'records' or 'answers' "
                f"from message (Call ID: {message.call_id}) using input_mappings. Extracted: {extracted_data.keys()}. Skipping scoring."
            )
            return
