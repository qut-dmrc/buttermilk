...
from typing import Literal, Type # For type hinting
...
class Reasons(BaseModel):
    ...
    def __str__(self) -> str:
        ...
        return (
            ...
            f"**Reasoning Steps:**\n\t{reasons_str if reasons_str else 'No specific reasons provided.'}"
        )
...
class Judge(LLMAgent):
    ...
    async def evaluate_content(
        ...
        """
        # TODO: Clarify the role of @buttermilk_handler. If this method is a primary
        # way Judge is invoked, the NotImplementedError should be removed.
        # If invocation is always via agent.invoke() -> _process(), this handler
        # might be for a specific Autogen integration pattern or can be removed if unused.
        raise NotImplementedError(
            "@buttermilk_handler for Judge.evaluate_content is conceptual or for specific adapter use. "
            "Typical invocation is via agent.invoke() which calls the agent's _process() method."
        )
        
        # The following logic would be executed if the handler were fully implemented
        # and called directly by a compatible message routing system.
        logger.debug(f"Judge agent '{self.agent_name}' received evaluation request via evaluate_content handler.")
        
        # Delegate to the LLMAgent's _process method.
        # This handles prompt templating, LLM call, retries, and parsing into _output_model (JudgeReasons).
        # _process returns an AgentOutput (or subclass like AgentTrace if super calls invoke)
        agent_output_result: AgentOutput = await self._process(message=message) # type: ignore # _process is on LLMAgent

        # Wrap the result in an AgentTrace if it isn't already.
        # If _process from LLMAgent was called directly (not via self.invoke),
        # agent_output_result might be a simpler AgentOutput.
        # AgentTrace requires more fields like session_id, agent_info, inputs.
        if not isinstance(agent_output_result, AgentTrace):
            trace = AgentTrace(
                agent_id=self.agent_id, # From base Agent
                session_id=self.session_id, # From base Agent
                agent_info=self._cfg, # From base Agent (_cfg is AgentConfig)
                inputs=message, # The original input message
                outputs=agent_output_result.outputs, # Outputs from LLM (parsed as JudgeReasons)
                metadata=agent_output_result.metadata, # Metadata from LLM call
                call_id=agent_output_result.call_id, # Call ID from AgentOutput
                # error field from FlowMessage base if any errors occurred in _process
                error=getattr(agent_output_result, "error", []) 
            )
            return trace
        else:
            return agent_output_result # It's already an AgentTrace
