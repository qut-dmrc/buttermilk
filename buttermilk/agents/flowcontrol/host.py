...
class HostAgent(Agent):
    ...
    max_user_confirmation_time: int = Field(
        default=240, # Total time, not per request
    ...
    async def _wait_for_user(self, step: StepRequest) -> bool:
        ...
        Returns:
            bool: `True` if the user confirmed the step within the allowed time,
                  `False` if the user rejected the step, did not respond in time,
                  or if `callback_to_groupchat` is not set.
        """
        if not self.callback_to_groupchat:
            logger.error(f"Host '{self.agent_name}': Cannot wait for user, callback_to_groupchat is not set. Assuming rejection.")
            return False

        # Calculate number of 60-second tries based on max_user_confirmation_time
        # Ensure at least one try if max_user_confirmation_time is positive.
        num_tries = max(1, self.max_user_confirmation_time // 60)

        for try_num in range(num_tries):
            logger.debug(f"Host '{self.agent_name}': Waiting for user confirmation for step '{step.role}' (Attempt {try_num + 1}/{num_tries}).")
            await self.request_user_confirmation(step) # Send/resend confirmation request
            try:
                await asyncio.wait_for(self._user_confirmation_received.wait(), timeout=60.0)
                # Event was set, now check the actual confirmation content
                if self._user_confirmation and self._user_confirmation.confirm:
                    logger.info(f"Host '{self.agent_name}': User confirmed step: {step.role}")
                    return True
                else: # Event set but confirm is False or None (e.g., user sent feedback without explicit confirm)
                    logger.info(f"Host '{self.agent_name}': User did not confirm or rejected step: {step.role}. User message: {self._user_confirmation}")
                    # If user explicitly rejects (e.g. confirm=False), or sends other non-confirming message.
                    # Depending on desired logic, an explicit reject (confirm=False) might immediately stop.
                    # For now, any non-True confirmation after event is treated as non-confirmation for this attempt.
                    # If halt is requested by user, end the flow.
                    if self._user_confirmation and self._user_confirmation.halt:
                        logger.info(f"Host '{self.agent_name}': User requested to HALT the flow.")
                        await self.callback_to_groupchat(StepRequest(role=END, content="User requested to halt the flow."))
                        return False # Signal to stop flow
                    return False # User did not confirm this step
            except asyncio.TimeoutError:
                logger.warning(f"Host '{self.agent_name}': Timeout (60s) waiting for user confirmation for step '{step.role}'. Retrying if attempts left.")
                # Continue to next attempt if any remain
        
        # All attempts exhausted
        timeout_msg = f"Host '{self.agent_name}': User did not respond to confirm step '{step.role}' within the total allowed time ({self.max_user_confirmation_time}s). Ending flow."
        logger.error(timeout_msg)
        await self.callback_to_groupchat(StepRequest(role=END, content=timeout_msg))
        return False
            ...
            while True: # Loop indefinitely until cancelled
                ...
                async with self._tasks_condition: # Ensure thread-safe access to pending tasks
                    if self._pending_tasks_by_agent:
                        progress_update_msg = TaskProgressUpdate(
                            source=self.agent_id, # This HostAgent is the source of the progress update
                            status="in_progress", # Overall status while waiting
                            step_name=self._current_step, # Name/role of the step being executed
                            waiting_on=dict(self._pending_tasks_by_agent), # Snapshot of pending tasks
                            message=f"Waiting for {len(self._pending_tasks_by_agent)} agent(s) to complete current step."
                        )
                        logger.debug(f"Host '{self.agent_name}': Sending periodic progress update: {progress_update_msg.waiting_on}")
                        if self.callback_to_groupchat:
                            await self.callback_to_groupchat(progress_update_msg)
                        else: # Should not happen if initialized correctly
                            logger.error(f"Host '{self.agent_name}': callback_to_groupchat not set, cannot send progress update.")
                    # else: No pending tasks, no need to report.
        except asyncio.CancelledError:
            logger.debug(f"Host '{self.agent_name}': Progress reporting task was cancelled.")
        except Exception as e: # Catch any other unexpected errors
            logger.exception(f"Host '{self.agent_name}': Error in periodic progress reporting task: {e!s}")


    async def _wait_for_all_tasks_complete(self) -> bool:
        """Waits until all currently pending tasks are reported as complete.
        ...
        """
        progress_reporter_task: asyncio.Task[None] | None = None
        try:
            async with self._tasks_condition:
                # Start the periodic progress reporter task if there are pending tasks or if we expect tasks for a new step
                if self._pending_tasks_by_agent or self._step_starting.is_set():
                    logger.info(
                        f"Host '{self.agent_name}': Waiting for tasks. Pending: {list(self._pending_tasks_by_agent.keys())}. "
                        f"Step starting: {self._step_starting.is_set()}."
                    )
                    progress_reporter_task = asyncio.create_task(self._report_progress_periodically())
                else: # No tasks pending and step not marked as starting
                    logger.info(f"Host '{self.agent_name}': No pending tasks and step not starting. Proceeding without wait.")
                    return True # No tasks to wait for

                # The predicate for wait_for:
                # 1. self._step_starting.is_set() is False: Ensures that the current step's
                #    initialization/dispatch phase (where _step_starting might be True) is over.
                # 2. self._pending_tasks_by_agent is empty: All tasks that were started have completed.
            ...
            logger.exception(f"Host '{self.agent_name}': Unexpected error during task completion wait: {e!s}")
            self._step_starting.clear()  # Ensure event is cleared on error
            return False
        finally:
            if progress_reporter_task: # Ensure cancellation of the reporter task
                progress_reporter_task.cancel()
                try:
                    await progress_reporter_task
                except asyncio.CancelledError:
                    logger.debug(f"Host '{self.agent_name}': Progress reporter task successfully cancelled.")
    ...
    async def _run_flow(self, message: ConductorRequest) -> None:
        ...
        # Initialize participants from the ConductorRequest
        if not message.participants: # message.participants is Mapping[str, str] (role_name: description)
            err_msg = f"Host '{self.agent_name}': Received ConductorRequest with no participants. Aborting flow."
            logger.error(err_msg)
            if self.callback_to_groupchat: # Check if callback is set
                await self.callback_to_groupchat(StepRequest(role=END, content=err_msg))
            raise FatalError(err_msg) # Critical failure if no participants
        
        # Store participant details (e.g., their configurations or just roles)
        # self._participants is dict[str, Any]. Value could be AgentVariants from ConductorRequest if it held them.
        # For now, assuming message.participants is role_name: description as per ConductorRequest docstring.
        # If ConductorRequest.participants was meant to be Mapping[str, AgentVariants], that needs adjustment.
        self._participants = dict(message.participants) 
        logger.info(f"Host '{self.agent_name}': Participants for this flow: {list(self._participants.keys())}")

        # Initialize the step generator now that participants are known
        ...
        async for next_step_request in self._step_generator:
            ...
            if not await self.wait_check_last_step_completions():
                logger.error(f"Host '{self.agent_name}': Failed to complete tasks from previous step. Ending flow.")
                await self._execute_step(StepRequest(role=END, content="Flow ended due to previous step failure or timeout."))
                break # Exit flow

            # If the next step is END, we might not need user confirmation.
            if next_step_request.role == END:
                await self._execute_step(next_step_request)
                break # End of flow

            # 2. If human_in_loop, wait for user confirmation for the CURRENT step.
            if self.human_in_loop:
                if not await self._wait_for_user(next_step_request):
                    logger.info(f"Host '{self.agent_name}': User did not confirm step '{next_step_request.role}' or requested halt. Ending flow.")
                    # _wait_for_user sends END signal if user halts/times out.
                    # If it returns False due to simple rejection, we might also send END or a specific message.
                    # For now, assume _wait_for_user handles signaling END appropriately.
                    break # Exit flow

            # 3. Execute the current step.
            await self._execute_step(next_step_request)
            
            # If the executed step was END (e.g., forced by user or logic), break.
            if next_step_request.role == END: 
                break
