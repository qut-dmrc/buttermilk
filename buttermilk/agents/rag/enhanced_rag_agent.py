...
class EnhancedRagAgent(Agent):
    ...
    enable_query_planning: bool = Field(
        default=True,
        description="Enable LLM-driven query planning and analysis"
    )
    enable_result_synthesis: bool = Field(
        default=True,
        description="Enable LLM-driven result synthesis and ranking"
    )
    search_strategies: List[str] = Field(
        default_factory=lambda: ["semantic", "title", "summary", "hybrid"],
        description="Available search strategies for this agent"
    )
    max_search_rounds: int = Field(
        default=3,
        description="Maximum number of search strategies to try"
    )
    planning_model: Optional[str] = Field(
        default=None,
        description="LLM model to use for query planning (defaults to main model)"
    )
    synthesis_model: Optional[str] = Field(
        default=None,
        description="LLM model to use for result synthesis (defaults to main model)"
    )
    ...
    include_search_explanation: bool = Field(
        default=True,
        description="Include explanation of search strategy in response"
    )
        ...
        try:
            ...
            if not query:
                return AgentOutput(
                    ...
                    outputs="No query provided. Please provide a search query.",
