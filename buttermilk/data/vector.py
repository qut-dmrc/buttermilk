...
<<<<<<< HEAD
    deduplication_strategy: Literal["record_id", "content_hash", "both", "none"] = "both"
=======
...
class ChromaDBEmbeddings(VectorStorageConfig):
    ...
    embeddings_cache_dir: str = Field(
        ...
        description="Subdirectory within cache_dir for embeddings. Defaults to cache.EMBEDDINGS constant for consistency.",
    )
...
<<<<<<< HEAD
    ...
    deduplication_strategy: Literal["record_id", "content_hash", "both", "none"] = Field(default="both")
=======
...
>>>>>>> origin/stable
    ...
    read_only: bool = Field(
        default=False,
        description="Read-only mode disables all write and sync operations",
    )
...
<<<<<<< HEAD
                    ...
                    if self.read_only:
                        logger.info("   🔢 Total embeddings: (count skipped in read-only mode)")
                    else:
...
=======
                    ...
                    if self.read_only:
                        logger.info(
                            "   🔢 Total embeddings: (count skipped in read-only mode)"
                        )
                    else:
                        total_embeddings = await asyncio.to_thread(
...
=======
                logger.info(
                    f"✅ Collection '{self.collection_name}' ready (count skipped in read-only mode)"
                )
            else:
                count = await asyncio.to_thread(existing_collection.count)
                logger.info(
...
<<<<<<< HEAD
                logger.debug(f"Upserting batch {i // CHROMA_MAX_BATCH_SIZE + 1}: chunks {i}-{batch_end - 1} ({batch_size} items)")
=======
                logger.debug(
                    f"Upserting batch {i // CHROMA_MAX_BATCH_SIZE + 1}: chunks {i}-{batch_end - 1} ({batch_size} items)"
                )
>>>>>>> origin/stable
...
>>>>>>> origin/stable
    ...
    async def _should_skip_record(  # noqa: PLR0911, PLR0912
        self,
        record: Record,
        force_reprocess: bool = False,
...
>>>>>>> origin/stable
        ...
        if strategy == "both":
            ...
            if ids:
                ...
                return (
                    True,
                    "identical content (hash) already embedded under different record_id",
                )
