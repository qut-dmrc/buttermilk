    ...
    **parameters: Any,
    ...
    if "schema" in parameters and "dataset" in parameters:
        try:
            ...
            logger.error(
                f"Failed to upload data to BigQuery (schema/dataset provided). Error: {e!s}",
                exc_info=True,
            )
    ...
    if final_uri_str and final_uri_str.startswith(
        ...
        if isinstance(data, pd.DataFrame):
            ...
            file_upload_methods.append(
                lambda **kwargs_lambda: upload_dataframe_json(
                    data=data,
                    uri=final_uri_str,
                    **kwargs_lambda.get("kwargs_for_upload", {}),
                )
            )  # type: ignore
...
def upload_dataframe_json(data: pd.DataFrame, uri: str, **kwargs: Any) -> str:
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Pandas to_json fallback also failed for {uri}: {e_pandas!s}",
                exc_info=True,
            )
...
def upload_rows(
    ...
    **parameters: Any,  # Catch-all for other params, not used directly here
    ...
    logger.debug(
        f"Inserting {len(bq_prepared_rows)} rows into BigQuery table {final_dataset}.",
        n_rows=len(bq_prepared_rows),
        table=final_dataset,
    )
    ...
    if not all_errors:
        logger.debug(
            f"Successfully pushed {len(bq_prepared_rows)} rows to BigQuery table {final_dataset}.",
            n_rows=len(bq_prepared_rows),
            table=final_dataset,
        )
    else:
        ...
        logger.error(
            f"Errors during BigQuery upload to {final_dataset}: {error_summary}",
            table=final_dataset,
