...
class ChatParser(BaseModel):
    ...
    def parse(self, text: str) -> Any:
        """Parses a string, attempting to extract and decode a JSON object.
        ...
        try:
            ...
            error_message = f"Unable to decode JSON in text. Error: {e.message}. Problematic text (approx. pos {e.pos}): '{text[max(0,e.pos-20):e.pos+20]}'"
            if self.on_error == "raise":
                logger.error(error_message)
                raise
            elif self.on_error == "warn":
                logger.warning(error_message)
            # For 'ignore' or 'warn', return original text wrapped
            return {"response": text, "error": error_message}
        except Exception as e_other: # Catch other unexpected errors during extraction/parsing
            error_message = f"Unexpected error during JSON parsing: {e_other!s}"
            if self.on_error == "raise":
                logger.error(error_message, exc_info=True)
                raise JSONDecodeError(error_message, text, 0) from e_other
            elif self.on_error == "warn":
                logger.warning(error_message, exc_info=True)
            return {"response": text, "error": error_message}


        # If parsing resulted in something other than a dict (e.g., a list from a JSON array)
        # wrap it in a dict for consistency, unless it's None.
        if parsed_output is not None and not isinstance(parsed_output, dict):
            logger.warning(f"Parsed JSON output is not a dictionary (type: {type(parsed_output)}). Wrapping in {{'response': ...}}.")
            parsed_output = {"response": parsed_output}
        elif parsed_output is None: # If parsing somehow resulted in None (e.g. "null" string)
             logger.warning("Parsed JSON output is None. Returning as {'response': null}.")
             parsed_output = {"response": None}
