...
async def download_and_convert(
    ...
    **kwargs: Any, # Additional metadata for the Record
    ...
    if html or (isinstance(obj, str) and (active_mime and active_mime.startswith("text/html"))):
        ...
        # 'plain_text' from readabilipy is often the primary content
...
def extract_main_content_bs(html: bytes | str) -> str:
    """Extracts and cleans the main textual content from an HTML string or bytes using BeautifulSoup.

    This function attempts to isolate the main readable content of a webpage by:
    1.  Removing common non-content HTML elements like `<script>`, `<style>`,
        `<header>`, `<footer>`, `<nav>`, `<iframe>`, and `<aside>`.
    2.  Searching for common main content container elements (e.g., `<main>`,
        `<article>`, or elements with IDs/classes containing "content", "main",
        or "article").
    3.  If no specific main container is found, it falls back to using the `<body>`.
    4.  Extracts all text from the determined main content area.
    5.  Cleans up extraneous whitespace from the extracted text.

    Args:
        html (bytes | str): The HTML content, either as a bytes string (which
            will be decoded, assuming UTF-8) or a standard string.

    Returns:
        str: A single string containing the cleaned textual representation of the
             main content found on the page.
    """
    soup = BeautifulSoup(html, "html.parser") # Create a BeautifulSoup object

    # Remove common non-content elements
    for element_type in soup.find_all(
        ["script", "style", "header", "footer", "nav", "iframe", "aside"]
    ):
    ...
    if not main_content_element:
        main_content_element = soup.body if soup.body else soup # Use soup itself if body is also missing
