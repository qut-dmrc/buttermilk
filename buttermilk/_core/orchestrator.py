...
from buttermilk._core.contract import FlowMessage # Core message contract
...
from buttermilk._core.types import ( # Core data types
    Record,
    RunRequest,
)
...
class Orchestrator(OrchestratorProtocol, ABC):
    ...
    async def run(self, request: RunRequest) -> None:
        ...
        """
        logger.info(f"Starting run for orchestrator '{self.name}' with request '{request.name}'.")

        # Define attributes for Weave tracing.
        # The operation name for Weave will combine the orchestrator's configured name and the request's name.
        op_name = f"{self.name} {request.name}".strip()
        # Using self._run as the operation being traced.
        op_ref = weave.op(self._run, call_display_name=op_name) # type: ignore # weave.op expects a function
        
        # Create the main trace call for this orchestrator run.
        # Inputs are serialized from the RunRequest.
        # Attributes provide additional metadata for the trace.
        orchestrator_trace = bm.weave.create_call(
            op_or_func=op_ref, # Pass the Weave operation reference
            inputs=request.model_dump(mode="json"),
            display_name=f"{self.orchestrator} {op_name}", # More descriptive display name
            attributes=request.tracing_attributes,
        )
        self.trace_id = orchestrator_trace.trace_id # Store trace_id for potential internal use
    ...
    async def get_record_dataset(self, record_id: str) -> Record:
        ...
        for source_name, dataset in self._data_sources.items():
            ...
            try:
                ...
                elif found_records_df.shape[0] > 1:
                    raise ValueError(
