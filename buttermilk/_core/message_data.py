...
def extract_message_data(
    ...
    if not input_mappings or not isinstance(input_mappings, dict):
        logger.debug(f"No valid input mappings provided")
        return extracted
...
def extract_records_from_data(data: dict[str, Any]) -> list[Record]:
    """
    Extract Record objects from the 'records' field in the extracted data.
    
    Args:
        data: Dictionary containing extracted data, potentially with 'records' key
