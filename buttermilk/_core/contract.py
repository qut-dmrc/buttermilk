...
class AgentOutput(BaseModel):
    ...
    @classmethod
    def _ensure_call_id(cls, v: str | None) -> str:
        """Ensures `call_id` is set, attempting to use Weave call ID if `v` is not provided.

        Args:
            v: The explicitly provided value for `call_id`.

        Returns:
            str: A valid call ID.
        """
        if not v: # If no call_id is provided
            try:
                # Attempt to get current Weave call ID
                import weave as wv_internal # Local import
                call = wv_internal.get_current_call()
                if call and hasattr(call, "id") and call.id:
                    return str(call.id)
            except ImportError:
                logger.debug("Weave library not installed. Cannot use Weave call ID for AgentOutput.")
            except Exception as e:
                logger.debug(f"Could not get Weave call ID for AgentOutput: {e!s}")
            # Fallback to generating a new shortuuid if Weave ID is not available
            return str(shortuuid.uuid())
        return v # Return provided value
...
class AgentTrace(FlowMessage, AgentOutput):
    ...
    def model_dump(self, **kwargs: Any) -> dict[str, Any]:
        """Overrides `model_dump` to exclude fields with empty collections by default.
        ...
        """
        # Initialize exclude_keys set from kwargs or as an empty set
        exclude_keys = set(kwargs.pop("exclude", set()) or set())


        # Identify keys with empty collections in the current instance's __dict__
        # This is a bit fragile as it relies on __dict__ structure
        for key, value in self.__dict__.items():
            if isinstance(value, (list, dict, set)) and not value: # Check if it's empty
                exclude_keys.add(key)
        
        # Add identified keys to kwargs['exclude']
        kwargs["exclude"] = exclude_keys if exclude_keys else None # Pydantic expects None if no exclusions

        return super().model_dump(**kwargs)

    def model_dump_json(self, **kwargs: Any) -> str:
        """Overrides `model_dump_json` to ensure `exclude_none` and `exclude_unset` are true.
        
        This also incorporates the logic from the custom `model_dump` to exclude
        empty collections when serializing to JSON.

        Args:
            **kwargs: Keyword arguments passed to Pydantic's `model_dump_json`.

        Returns:
            str: The JSON string representation of the model.
        """
        # Get the dictionary representation with empty collections excluded
        # by calling the overridden model_dump
        dump_dict_kwargs = kwargs.copy() # To avoid modifying original kwargs for super().model_dump_json
        # We need to ensure that the 'exclude' logic from model_dump is applied before serializing to JSON
        # However, model_dump_json calls model_dump internally. So, we ensure our exclude logic is passed.
        
        current_exclude = set(kwargs.get("exclude", set()) or set())
        for key, value in self.__dict__.items():
            if isinstance(value, (list, dict, set)) and not value:
                current_exclude.add(key)
        
        final_kwargs = {
            **kwargs,
            "exclude": current_exclude if current_exclude else None,
            "exclude_none": kwargs.get("exclude_none", True),
            "exclude_unset": kwargs.get("exclude_unset", True),
        }
        
        return super().model_dump_json(**final_kwargs)
