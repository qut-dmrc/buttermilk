...
class Agent(AgentConfig, ABC):
    ...
    session_id: str = Field(
        ...,
        description="A unique session ID for this specific flow execution. This ID "
                    "is crucial for tracing and correlating activities within a "
    ...
    async def __call__(
        ...
        **kwargs: Any,
        ...
        # --- Execute Core Logic ---
        try:
            # Construct trace parameters, ensuring no None values for Weave attributes
            trace_params = {
                "agent_name": self.agent_name,
                "model": self._cfg.parameters.get("model") if self._cfg.parameters else None,
                **(message.parameters or {}),
                **(message.metadata or {}),
                **(self.parameters or {}), # self.parameters is from AgentConfig
            }
            # Filter out None values from trace_params as Weave attributes cannot be None
            trace_params_filtered = {k: v for k, v in trace_params.items() if v is not None}


            child_call = bm.weave.create_call(
                op_or_func=self._process,  # Pass the method directly
                inputs=message.model_dump(mode="json"), # Pass inputs to _process via message arg
                parent=parent_call,
                display_name=self.agent_name,
                attributes=trace_params_filtered,
            )
            if parent_call and hasattr(parent_call, "_children") and isinstance(parent_call._children, list):
                 parent_call._children.append(child_call) # Nest this call for tracing visualization

            result = await self._process(message=message) # Pass the message argument to _process
            if result and child_call: # Ensure result and child_call exist
                result.call_id = child_call.id # Assign call_id to the output
            return result # type: ignore # AgentOutput type is expected
        finally:
            if child_call:
                ...
                bm.weave.finish_call(child_call, output=result if result else None, op=self._process)
    ...
    async def invoke(
        ...
        **kwargs: Any,
        ...
        try:
            processed_result = await self.__call__(message=final_input)
        except Exception as e:
        ...
        # Ensure processed_result is not None before proceeding
        if processed_result is None:
            logger.error(f"Agent {self.agent_name} __call__ returned None, which is unexpected.")
            processed_result = ErrorEvent(source=self.agent_name, content="Agent returned None unexpectedly.")
            is_error = True
        ...
        await public_callback(
            TaskProcessingComplete(
                ...
                role=self.role,
                task_index=0,  # Assuming single task for now
                more_tasks_remain=False, # Assuming single task for now
                is_error=is_error,
            ),
        )

        trace = AgentTrace(
            call_id=getattr(processed_result, 'call_id', None), # Get call_id from result
            session_id=self.session_id,
            agent_id=self.agent_id,
            agent_info=self._cfg, # Use the AgentConfig snapshot
            inputs=final_input,
            parent_call_id=final_input.parent_call_id,
        )
        
        # Extract outputs for the trace. If it was an ErrorEvent, content is the error.
        if isinstance(processed_result, ErrorEvent):
            trace.outputs = {"error": processed_result.content}
        elif hasattr(processed_result, 'outputs'):
            trace.outputs = processed_result.outputs
        else: # Fallback if result has no 'outputs' attribute
            trace.outputs = {"result": str(processed_result)}


        logger.debug(f"Agent {self.agent_name} finished task for message: {message.message_id if message else 'Unknown'}.")
    ...
    async def _check_heartbeat(self, timeout: float = 240.0) -> bool:
        ...
        try:
            ...
            return True
        except asyncio.TimeoutError: # More specific exception for timeout
