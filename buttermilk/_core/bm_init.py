...
class BM(SessionInfo):
    ...
    def setup_logging(self, verbose: bool = False) -> None:
        ...
        if self.logger_cfg and self.logger_cfg.type == "gcp":
            if not hasattr(self.logger_cfg, "project") or not getattr(self.logger_cfg, "project"):
                raise RuntimeError("GCP logger configuration (logger_cfg.project) is missing or empty.")
            if not hasattr(self.logger_cfg, "location") or not getattr(self.logger_cfg, "location"):
        ...
        if self.logger_cfg and self.logger_cfg.type == "gcp":
            ...
            # Get GCS log client via CloudManager
            gcs_log_client = self.cloud_manager.gcs_log_client(self.logger_cfg)
            if gcs_log_client:
                cloud_handler = CloudLoggingHandler(
                    client=gcs_log_client,
                    resource=cloud_logging_resource,
                    name=f"buttermilk-{self.name}-{self.job}", # Log name in Cloud Logging
                    labels=self.model_dump(include={"run_id", "name", "job", "platform"}, exclude_none=True),
                )
                cloud_handler.setLevel(logging.INFO)  # Cloud logs typically at INFO, not DEBUG
                logger.addHandler(cloud_handler)
            else:
                logger.warning("Failed to initialize GCS log client for Cloud Logging.")
    ...
    def save(
        ...
        **kwargs: Any,
        ...
        effective_extension = extension if extension else ".json"
