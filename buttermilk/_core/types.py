            ...
            if isinstance(content, str):
                ...
                if is_b64(content):
                    if self._text: # If text was already set, this is ambiguous
                         raise ValueError("MediaObj received string content that is also valid base64, and text was already set.")
...
class Record(BaseModel):
    ...
    def __str__(self) -> str:
        """Returns the combined textual representation of the record (from `self.text`)."""
        return self.text
    ...
    @property
    def text(self) -> str:
        """Generates a combined textual representation of the record.
        ...
        if self.metadata:
            ...
            for key, value in self.metadata.items():
                if key not in ["title", "fetch_timestamp_utc", "fetch_source_id", "components"]: # Exclude some common internal/structural keys
    ...
    def as_message(self, role: Literal["user", "assistant"] = "user") -> UserMessage | AssistantMessage:
        ...
        """
        if role == "assistant":
            # For assistant messages, typically the combined text representation is used.
...
class RunRequest(BaseModel):
    ...
    record_id: str | None = Field(
        default="", description="Optional ID of a specific record to look up and process."
    )
    ...
    uri: str | None = Field(
        default="", description="Optional URI (e.g., URL, file path) to fetch initial data or a record from."
    )
    records: list[Record] = Field(
        default_factory=list, description="Input `Record` objects for the flow, potentially including ground truth."
    )
    parameters: dict[str, Any] = Field( # Added type hint for dict value
        default_factory=dict, description="Additional parameters to customize flow execution."
    )

    # Fields for client interaction, typically excluded from persisted state
    callback_to_ui: Any | None = Field(
        default=None, exclude=True, description="Optional callback function for sending updates to a UI."
    )
    ui_type: str = Field(
        ..., description="Type of UI initiating the run (e.g., 'cli', 'api'). Excluded from serialization.", exclude=True
    )

    # Batch processing specific fields
    batch_id: str | None = Field(
        default=None, description="ID of the parent batch, if this run is part of a batch."
    )
    status: str = Field(
        default="pending", description="Current status of this job (e.g., 'pending', 'running', 'completed')."
    )
    ...
    started_at: str | None = Field(
        default=None, description="Timestamp (ISO format UTC) when execution started."
    )
    completed_at: str | None = Field(
        default=None, description="Timestamp (ISO format UTC) when execution completed."
    )
    error: str | None = Field(default=None, description="Error message if the job/run failed.")
    result_uri: str | None = Field(
        default=None, description="URI pointing to the results of the job/run, if applicable."
    )

    # API-specific fields (can be used by other interfaces too)
    source: list[str] = Field(
        default_factory=list, description="List of source identifiers, e.g., for API requests indicating data origins."
    )
    mime_type: str | None = Field(
        default=None, description="MIME type for input data, especially if 'data' field is used."
    )
    data: bytes | None = Field(
        default=None, description="Raw binary data input, e.g., for file uploads via API."
    )
