...
from datatools.utils import read_file
...
class ImageRecord(BaseModel):
    ...
    def as_base64(self, longest_edge=None, shortest_edge=None):
        return image_to_b64(
            self.image, longest_edge=longest_edge, shortest_edge=shortest_edge
        )
    ...
    def save(self, path: Union[str, CloudPath, Path]) -> str:
...
def read_image(
    ...
    image_b64 = image_to_b64(
        img, longest_edge=longest_edge, shortest_edge=shortest_edge
    )
    data.seek(0)  # go to the start of the image again
    img = ImageRecord(
        image=img, uri=path, image_b64=image_b64, _image_bytes=data.read()
    )
